    ...
    <div className="bg-slate-50 min-h-screen pt-[136px] pb-20">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-10 text-center sm:text-left flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-3xl font-extrabold text-slate-900 leading-tight">Đơn hàng của tôi</h1>
            <p className="text-slate-500 text-sm mt-1">Quản lý và theo dõi tiến độ các đơn hàng phụ kiện Think Tank của bạn.</p>
          </div>
          <Link
            to="/products"
            className="bg-white hover:bg-slate-50 text-slate-700 font-bold px-5 py-2.5 rounded-xl border border-slate-200 hover:border-slate-350 transition-colors text-sm flex items-center gap-1.5 justify-center"
          >
            <ShoppingBag size={16} />
            Tiếp tục mua sắm
          </Link>
        ...
        {orders.length > 0 ? (
          <div className="space-y-6">
            {orders.map((order) => {
              ...
              return (
                <div key={order.id} className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden transition-all duration-300">
                  <div
                  ...
                  >
                    <div className="grid grid-cols-2 sm:flex sm:items-center gap-x-4 gap-y-2">
                      <div>
                        <span className="text-[10px] uppercase font-bold text-slate-400 block">Mã đơn hàng</span>
                        <span className="font-extrabold text-sm text-blue-600 tracking-wider">TT-{order.id}</span>
                      </div>
                      <div className="sm:border-l border-slate-100 sm:pl-4">
                        <span className="text-[10px] uppercase font-bold text-slate-400 block">Ngày đặt</span>
                        <span className="text-xs font-semibold text-slate-700">{formattedDate}</span>
                      </div>
                      <div className="sm:border-l border-slate-100 sm:pl-4">
                        <span className="text-[10px] uppercase font-bold text-slate-400 block">Tổng tiền</span>
                        <span className="text-xs font-extrabold text-slate-900">{(order.finalAmount || order.total || 0).toLocaleString('vi-VN')}đ</span>
                      </div>
                    </div>
                  ...
                  {isExpanded && (
                    <div className="border-t border-slate-50 bg-slate-50/30 p-5 sm:p-6 space-y-6 animate-in slide-in-from-top-2 duration-300">
                      <div>
                        <h4 className="font-bold text-slate-800 text-xs uppercase tracking-wider mb-3.5 flex items-center gap-1.5">
                      ...
                      <div className="grid sm:grid-cols-2 gap-6 pt-4 border-t border-slate-100">
                        ...
                        {order.notes && (
                          <div className="bg-slate-50 border border-slate-100 p-4 rounded-2xl">
                            <h4 className="font-bold text-slate-700 text-xs uppercase tracking-wider mb-2.5 flex items-center gap-1.5">
                              <Package size={14} />
                              Ghi chú & Hóa đơn
                            </h4>
